function net = cnnff(net, x)
    ...
    for l = 2 : n   %  for each layer
        if strcmp(net.layers{l}.type, 'c') % 卷积层
            ...
            for j = 1 : net.layers{l}.outputmaps   %  for each output map
                ...
                %{
                [OriginalRow, OriginalCol, OriginalNum] = size(net.layers{l - 1}.a{1});
                if OriginalRow / OriginalCol ~= 0.5
                    error('输入样本维度错误');
                end
                z = zeros(OriginalRow - net.layers{l}.kernelsize + 1, (OriginalRow - net.layers{l}.kernelsize + 1)*2, OriginalNum);
                %}
                ...
                for i = 1 : inputmaps   %  for each input map
                    ...
                    %{
                    z1 = convn(net.layers{l - 1}.a{i}(:, 1:OriginalRow    , :), net.layers{l}.k{i}{j}, 'valid');
                    z2 = convn(net.layers{l - 1}.a{i}(:, OriginalRow+1:end, :), net.layers{l}.k{i}{j}, 'valid');
                    z = z + [z1, z2];
                    %}
                ...
                net.layers{l}.a{j} = 1 ./ (1 + exp(-(z + net.layers{l}.b{j})));%sigm(z + net.layers{l}.b{j});
            end
    ...
    net.o = 1 ./ (1 + exp(-(net.ffW * net.fv + repmat(net.ffb, 1, size(net.fv, 2)))));%sigm(net.ffW * net.fv + repmat(net.ffb, 1, size(net.fv, 2)));
