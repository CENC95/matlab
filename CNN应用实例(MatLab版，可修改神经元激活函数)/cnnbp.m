        ...
        if strcmp(net.layers{l}.type, 'c')
            for j = 1 : numel(net.layers{l}.a) % 该层特征map的个数
				...
				for k = 1:size(net.layers{l + 1}.d{j}, 3)
                    %%{
                    size1 = size(net.layers{l}.a{j}(:,:,k));
                    size2 = size(1 - net.layers{l}.a{j}(:,:,k));
                    size3 = size(kron(net.layers{l + 1}.d{j}(:,:,k), ones(net.layers{l + 1}.scale)) / net.layers{l + 1}.scale ^ 2);
                    if ~isequal(size1, size2) || ~isequal(size2, size3) || ~isequal(size3, size1)
                        dd=1;
                    end
                    %}
        ...
        elseif strcmp(net.layers{l}.type, 's')
            for i = 1 : numel(net.layers{l}.a) % 第l层特征map的个数
                ...
                for j = 1 : numel(net.layers{l + 1}.a) % 第l+1层特征map的个数
                    %{
                    row = size(net.layers{l + 1}.d{j}, 1);
                    z1  = convn(net.layers{l + 1}.d{j}(:, 1:row    , :), rot180(net.layers{l + 1}.k{i}{j}), 'full');
                    z2  = convn(net.layers{l + 1}.d{j}(:, row+1:end, :), rot180(net.layers{l + 1}.k{i}{j}), 'full');
                    z   = z + [z1, z2];
                    %}
        ...
        if strcmp(net.layers{l}.type, 'c')
            for j = 1 : numel(net.layers{l}.a)
                for i = 1 : numel(net.layers{l - 1}.a)
					% dk 保存的是 误差对卷积核 的导数
                    %{
                    Temp = flipall(net.layers{l - 1}.a{i});
                    row1 = size(Temp, 1);
                    row2 = size(net.layers{l}.d{j}, 1);
                    dk1  = convn(Temp(:, 1:row1    , :), net.layers{l}.d{j}(:, 1:row2    , :), 'valid');
                    dk2  = convn(Temp(:, row1+1:end, :), net.layers{l}.d{j}(:, row2+1:end, :), 'valid');
                    net.layers{l}.dk{i}{j} = [dk1, dk2] / size(net.layers{l}.d{j}, 3);
                    %}
